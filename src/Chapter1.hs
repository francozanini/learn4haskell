...
sumLast2 :: Int -> Int
sumLast2 n = secondToLastDigit + lastDigit n
  where
    tens = fst (divMod (abs n) 10)
    secondToLastDigit = lastDigit tens
...
firstDigit n
  | n `elem` [-9..9] = abs n
  | otherwise = firstDigit (dropLastDigit (abs n))
  where
    dropLastDigit x = fst (divMod x 10)
